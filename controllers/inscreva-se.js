...
exports.postInscrevasePage = async (req, res) => {
  ...
  await user.save();
  console.log(
    `[postInscrevasePage] email: ${user.email} | userId: ${user._id}`
  );

  const generated_verification_jwt = jwt.sign(
    _.pick(user, ["_id"]),
    process.env.JWT_VERIFICATION_EMAIL_SECRET,
    {
      expiresIn: 15 * 60,
    }
  );
  ...
  const emailVerificationLink = `${req.protocol}://${req.get(
    "host"
  )}/inscreva-se/verificar?conta=${generated_verification_jwt}`;
...
exports.getVerificarEmailLinkRoute = async (req, res) => {
  ...
  const verification_token = await VerificationToken.findOneAndDelete({
  ...
  res.render("inscreva-se__conta", {
    pathname: req.baseUrl,
    userId: foundUser._id,
  });
};

exports.postVerificarPage = async (req, res) => {
  ...
  const foundVerificationToken = await VerificationToken.findOneAndUpdate(
    { userId: userId },
    { token: generated_verification_jwt }
  );
...
exports.postInscrevaseContaPage = async (req, res) => {
  ...
  if (!user) {
    throw new Error(
      "[postInscrevaseContaPage] Could not find a user with this id"
    );
  }
...
exports.postInscrevasePrecarioPage = async (req, res) => {
  ...
  if (!user) {
    throw new Error(
      "[postInscrevasePrecarioPage] Could not find a user with this id"
    );
  }
